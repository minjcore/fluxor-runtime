...
func TestNewMainVerticleWithOptions_FailFast_EventBusFactoryError(t *testing.T) {
	_, err := NewMainVerticleWithOptions("", WithOptions(MainVerticleOptions{
...
func TestNewMainVerticleWithOptions_FailFast_UnknownProfile(t *testing.T) {
	_, err := NewMainVerticleWithOptions("", WithProfile("unknown-profile"))
	if err == nil {
		t.Fatalf("expected error for unknown profile")
	}
	if !strings.Contains(err.Error(), "unknown profile") {
		t.Errorf("error = %v, want containing 'unknown profile'", err)
	}
}
