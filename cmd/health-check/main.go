...
func main() {
	...
	if *url == "" {
		...
		flag.Usage()
		fmt.Fprintf(os.Stderr, "\nUsage: healthcheck -url <url> [options]\n")
		os.Exit(1)
	}

	if err := checkHealth(*url, *timeout, *format); err != nil {
...
func checkHealth(url string, timeout time.Duration, format string) error {
	...
	exitCode := 0
	if status == devops.HealthStatusUnhealthy {
		exitCode = 1
	}

	if err := outputHealthCheck(check, format, duration); err != nil {
		return err
	}
